package me.newfarminghosting.nl;
		...
		if(cmdl.equalsIgnoreCase("chatcolor")){
			...
			if(args.length == 0){
					...
					if(canChangeColor(player) || player.hasPermission("customChat.unlmited")){
						...
						player.sendMessage(ChatColor.AQUA + "I'm sorry, but you cannot change your chat color!");
	...
	public void playerChat(AsyncPlayerChatEvent event){
		...
		if(!(changingColor.contains(player.getName()))){
			...
			if(event.getMessage().length() != 1){
				...
				player.sendMessage(replaceColors(chatColor + "Your chat color has been setted up!"));
